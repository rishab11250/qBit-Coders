...
const PomodoroTimer = () => {
    ...
    return (
        <div
            className="fixed bottom-6 left-0 w-[360px] h-14 z-40 group"
            onMouseEnter={() => setIsHovered(true)}
            onMouseLeave={() => setIsHovered(false)}
        >
            <div className="relative w-20 h-full flex justify-center items-end">
                <motion.div
                    className={`${isExpanded ? 'absolute bottom-0 left-20 flex flex-col items-start justify-end' : 'relative'}`}
                    initial={false}
                    animate={{
